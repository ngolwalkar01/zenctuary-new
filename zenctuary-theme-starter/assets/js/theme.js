document.addEventListener('DOMContentLoaded', () => {
  const getAuthModal = () => document.getElementById('zenctuary-auth-modal');

  const fallbackSetAuthView = (view = 'login') => {
    ...
    modal.querySelectorAll('.zen-auth-view').forEach((panel) => {
      panel.classList.toggle('is-active', panel.dataset.view === view);
    });
  };

  const fallbackOpenAuthModal = (view = 'login') => {
    const modal = getAuthModal();
  ...
  if (!window.zenctuaryAuth || typeof window.zenctuaryAuth.openModal !== 'function') {
    window.zenctuaryAuth = {
      openModal: fallbackOpenAuthModal,
      closeModal: fallbackCloseAuthModal,
      setState: fallbackSetAuthView,
      syncUI: () => {}
    };
  }
  ...
  document.addEventListener('click', (event) => {
    ...
    if (authTrigger) {
      ...
      if (action === 'login' || action === 'signup') {
        ...
        window.zenctuaryAuth.openModal(action);
        return;
      }
    }

    if (event.target.closest('#zen-modal-close')) {
      event.preventDefault();
      window.zenctuaryAuth.closeModal();
      return;
    }

    const modal = getAuthModal();
    if (modal && event.target === modal) {
      fallbackCloseAuthModal();
      return;
    }

    const switchBtn = event.target.closest('[data-zen-switch-view]');
    if (switchBtn) {
      event.preventDefault();
      fallbackSetAuthView(switchBtn.dataset.zenSwitchView || 'login');
    }
  });

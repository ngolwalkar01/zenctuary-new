...
function BlockView( { attributes, setAttributes, selectCard } ) {
	...
	const blockProps = useBlockProps( {
		...
		style: {
			...
			'--zen-static-dropins-heading-intro-spacing': `${ attributes.headingIntroSpacing }px`,
		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Layout', 'zenctuary' ) } initialOpen>
					...
					<RangeControl label={ __( 'Heading/subtext spacing', 'zenctuary' ) } value={ attributes.headingIntroSpacing } onChange={ ( headingIntroSpacing ) => setAttributes( { headingIntroSpacing } ) } min={ 0 } max={ 140 } />
					<RangeControl label={ __( 'Header/cards spacing', 'zenctuary' ) } value={ attributes.headerBottomSpacing } onChange={ ( headerBottomSpacing ) => setAttributes( { headerBottomSpacing } ) } min={ 0 } max={ 140 } />
